...
int search(unsigned char *x, int m, unsigned char *y, int n) {
    ...
    while (pos < n) {
        ...
        if (V) {
            ...
            // pattern_pos = m - 1, next_verify - win_start = m - 1.
            while (pattern_pos >= next_verify_pos - window_start_pos) {
                ...
                // If the next position might give us a complete match, keep going:
                while (pattern_pos == m - 1 && next_verify_pos < n) {
                    if (x[pattern_pos] == y[next_verify_pos]) {
                        pattern_pos++;
                        next_verify_pos++;
                    }
                    if (pattern_pos == m) count++;
