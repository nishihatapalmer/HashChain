...
/*
 * this is the cleanest version of the code I've produced that implements the loop...else logic clearly.
 */
...
int search(unsigned char *x, int m, unsigned char *y, int n) {
    ...
    BEGIN_PREPROCESSING
    const int MQ1 = m - Q + 1;
    const int Hm = preprocessing(x, m, B); // Hm is the rolling hash value we see after processing the entire pattern.
    END_PREPROCESSING
