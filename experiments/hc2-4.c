...
inline int search_chain(int pos, const int end_second_qgram_pos, unsigned int H, const unsigned int Hm,
                         unsigned int V, unsigned int B[ASIZE], const unsigned char *x, const int m, const unsigned char *y, int *count)
{
    while (pos >= end_second_qgram_pos)
    {
        pos -= Q;
        H = (H << S2) + CHAIN_HASH(y, pos);
        if (!(V & FINGERPRINT(H))) return pos;
...
int search(unsigned char *x, int m, unsigned char *y, int n) {
    ...
    while (pos < n) {
        H = ANCHOR_HASH(y, pos);
        V = B[H & TABLE_MASK];
        if (V) pos = search_chain(pos, pos - MQQ, H, Hm, V, B, x, m, y, &count);
        pos += MQ1;
    }
    END_SEARCHING
