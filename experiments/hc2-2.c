    ...
    while (pos < n) {
        ...
        { // If the hash entry is not empty, we have a potential match for the anchor hash
            ...
            while (pos >= end_second_qgram_pos)
            {
                ...
                if (!(V & FINGERPRINT(H))) {
                    matched = 0; // unset the matched flag.
                    break; // no fingerprint - end chain processing.
                }
                V = B[H & TABLE_MASK]; // get the next value.
            }

            if (matched) // test that we matched or not - not required if we rewrite the control flow directly.
            {
