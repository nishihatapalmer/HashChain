...
inline void search_chain(int *pos, const int end_second_qgram_pos, int *count, unsigned int H, unsigned int Hm,
                         unsigned int V, unsigned int B[ASIZE], unsigned char *x, int m, unsigned char *y)
{
...
int search(unsigned char *x, int m, unsigned char *y, int n) {
    ...
    while (pos < n) {
        H = ANCHOR_HASH(y, pos);
        V = B[H & TABLE_MASK];
        if (V) search_chain(&pos, pos - MQQ, &count, H, Hm, V, B, x, m, y);
        pos += MQ1;
    }
    END_SEARCHING
