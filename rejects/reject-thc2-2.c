...
int search(unsigned char *x, int m, unsigned char *y, int n) {
    ...
    while (pos < n)
    {
        ...
        if (pos < n)
        {
            ...
            //TODO: logic not entirely correct.  if we break when pos < end_second_qgram_pos, we still execute this bit.

            //if (pos < end_second_qgram_pos) // We read back as far as we can.  Check that the rolling hash equals Hm and if so, verify a match.
            if (pos < end_second_qgram_pos) // We read back as far as we can.  Check that the rolling hash equals Hm and if so, verify a match.
            {
